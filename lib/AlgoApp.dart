...
class AlgoApp extends StatefulWidget {
  int timeQuantity;
  AlgoApp({Key key,this.timeQuantity}):super(key: key);
  @override
  _AlgoAppState createState() => _AlgoAppState(timeQuantityOut: timeQuantity);
}

class _AlgoAppState extends State<AlgoApp> {
  int timeQuantityOut ;
  _AlgoAppState({this.timeQuantityOut});
  ...
  bool error = false;
  String choiceText = "";
  TextEditingController controllerNum;
  Widget resWidget;
  ...
  Widget build(BuildContext context) {
    ...
    int timeQuantity = timeQuantityOut;
    ...
    return Builder(
        builder: (context) => Scaffold(
          appBar: AppBar(
            ...
            title: Text('Cpu Scheduling ',style: TextStyle(color: Colors.white),),
            centerTitle: true,
            actions: [
              IconButton(
                  ...
                  onPressed: (){
                    print("Tapped");
                    Navigator.of(context).push(MaterialPageRoute(builder: (context)=>SetTimeQuantity()));
                  })
            ],
          ),
          body: LayoutBuilder(
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    children: [
                      IntrinsicHeight(
                        child: Row(
                          ...
                          children: [
                            Flexible(
                              child: Column(
                                children: generateDataInputList(timeQuantity),
                              ),
                            ),
                                ...
                                child: Column(
                                  children: [
                                    const Padding(
                                      ...
                                      child: Text(
                                        "Data",
                                        style: TextStyle(fontSize: 22,fontWeight: FontWeight.w700,decoration: TextDecoration.underline,),
                                      ),
                                    ),
                                    Flexible(
                                      child: Builder(
                                        builder: (context) {
                                          ...
                                          switch (component) {
                                            case Component.CPU:
                                              return ProcessTable.fromProcessList(processes, "Arrival time", "Burst Time", (int index) => "P${index + 1}");
                      ...
                      LayoutBuilder(
                        builder: (context, constraints) => buildAlgoToggle(constraints,timeQuantity),
                      ),
                      const SizedBox(
                        height: 10,
                      ),
  ...
  ToggleButtons buildAlgoToggle(BoxConstraints constraints,int timeQuantity) {
  ...
  List<Widget> generateDataInputList(int timeQuantity) {
    List<Widget> inputList = List.generate(
      DataChoice.values.length,
          (index) => RadioListTile<DataChoice>(
        title: Text(index != 3 ? getData(DataChoice.values[index]) : "Create"),
        ...
        onChanged: (DataChoice value) {
          setState(() {
            ...
            for (int i = 0; i < selectedAlgo.length; i++) if (selectedAlgo[i]) runAlgo(i,timeQuantity);
          });
        },
      ),
    );
  ...
  void runAlgo(int algoIndex,int timeQuantity) {
    ...
    try {
      ...
      switch (component) {
        case Component.CPU:
          algoResult = runCpuAlgo(CpuAlgo.values[algoIndex], log, parseComputationProcesses(),timeQuantity);
          break;
        case Component.Memory:
